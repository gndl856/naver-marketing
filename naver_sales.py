...
st.title("🚀 네이버 실시간 업종별 알짜 키워드 분석")
st.write("월간 검색량 **2,000건 이상**인 키워드만 골라드립니다.")

tabs = st.tabs(["🎯 업종별 자동 분석", "⌨️ 직접 키워드 입력"])

with tabs[0]:
    st.subheader("분석할 업종을 선택하세요")
    cols = st.columns(6)
...
with tabs[1]:
    st.subheader("직접 입력 분석")
    custom_input = st.text_input("기준 키워드 (쉼표로 구분)", placeholder="예: 무릎보호대, 보호대추천")
...
if selected_industry:
    all_data = []
    with st.spinner(f'[{selected_industry}] 네이버 진짜 데이터 불러오는 중...'):
    ...
    if all_data:
        ...
        st.markdown(f"### ✅ {selected_industry} 실시간 분석 리스트")
        st.write("표 우측 상단의 화살표를 누르면 정렬이 가능합니다.")
        ...
        st.download_button("📥 분석 결과 엑셀로 저장하기", csv, f"{selected_industry}_분석.csv", "text/csv")
    else:
        st.info("검색량 2,000건 이상의 키워드를 찾지 못했습니다.")
